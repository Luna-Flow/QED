...
$
  t ::= x : tau | c : tau | t_1 t_2 | lambda x{:}tau. t
$
...
$
  ...
  /
  (" "Gamma ⊢ lambda x{:}tau_1. t : "fun"(tau_1, tau_2)" ")
$
...
For example, the intended shape of selected rules is:

- `REFL`: from any term $t$, derive $⊢ t = t$.
- `ASSUME`: from boolean term $p$, derive $p ⊢ p$.
- `TRANS`: from $A ⊢ s = t$ and $B ⊢ t = u$, derive $A union B ⊢ s = u$.
- `EQ_MP`: from $A ⊢ p = q$ and $B ⊢ p$, derive $A union B ⊢ q$.
